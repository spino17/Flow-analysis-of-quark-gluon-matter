...
def gaussian_func(x, mean, cov_mat):
    ...
    print(x - mean)
    print(mean.size())
    val = -1 / 2 * torch.mm(x - mean, torch.mm(torch.inverse(cov_mat), torch.transpose(x - mean, 0, 1)))
    return torch.exp(val).float() / torch.sqrt(torch.pow(2 * pi, num_dim) * torch.det(cov_mat)).float()
...
loc = torch.tensor([[2., 3.], [1000., 2000.]], requires_grad=True)
cov_mat = torch.tensor([[[1., 0.], [0., 1.]], [[1., 0.], [0., 1.]]], requires_grad=True)
