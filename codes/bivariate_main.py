...
def fit(train_loader, val_loader, num_dim, num_epochs, num_steps, learning_rate):
    ...
    lower_coeff = torch.randn(int(num_dim * (num_dim + 1) / 2))  # vectorized lower triangular matrix
    a = torch.randn(3, 1)
    b = torch.randn(3, 1)
    ...
    cov_mat = torch.matmul(cov_mat_l, torch.transpose(cov_mat_l, 1, 2))  # cholesky decomposition
    ...
    for epoch_index in range(num_epochs):
        ...
        for x, y in train_loader:
            ...
            print("Train loss: %.2f - Val loss: %.2f" % (train_loss / train_len, val_loss / val_len))
